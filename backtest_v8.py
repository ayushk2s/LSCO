"""
backtest_v8.py  --  Liquidation Zone Reversal  v8  (GAP-AWARE FILL MODE)
=========================================================================
Builds directly on v7 (1m EXECUTION MODE) with one targeted fix:

  GAP-AWARE FILLS  (the only change from v7)
  ══════════════════════════════════════════
  v7 filled stops/trail/targets at the EXACT stop level even when a 1m bar
  OPENED beyond that level (a gap-through).  In reality you cannot fill at
  the stop price if the market skipped past it — you fill at the bar OPEN.

  v8 rule:
    • Stops  (initial SL, trail stop):  if bar OPENS beyond the stop level
      → fill at bar OPEN (WORSE price, more slippage on the trade).
    • Targets (partial TP, hard TP):    if bar OPENS beyond the target level
      → fill at TARGET price (CONSERVATIVE — we do not assume a better-than-
        target execution on a favorable gap).

  Impact per forensic audit: ~1.6 % of trades, ~0.3–0.4 % of net PnL.
  Expected result: slight PnL reduction vs v7, still clearly net positive.

ALL v7 FIXES INHERITED
...
# ─── 1m trade executor — v8 GAP-AWARE ────────────────────────────────────────

def _exec_1m(df_1m, m_start, entry_px, direction,
              sl_px, partial_tp_px, hard_tp_px,
              trade_atr, qty, fee_side, slip_pct):
    """
    Run the full trade state machine on 1m candles with GAP-AWARE fills.

    Gap rules:
      Stops  (SL, trail): bar opens BEYOND stop  -> fill at bar open  (worse).
      Targets (pTP, hTP): bar opens BEYOND target -> fill at target    (conservative).

    Conservative tie-break (v6/v7 heritage): when trail AND hard_tp both
    trigger on the same normal bar, trail wins (worst case assumed).

    Returns (close_ts, gross, exit_px, total_fee, total_slip, final_state)
    or None if data ends before the trade closes.
    """
    half    = qty / 2
    state   = "full"
    partial_locked = 0.0
    ...
    for m_idx in range(m_start, n):
        ...
        if state == "full":
            ...
            if direction == "LONG":
                # GAP-AWARE SL: bar opened AT or BELOW stop -> fill at open (worse)
                if mr["open"] <= sl_px:
                    exit_px = mr["open"]
                    gross   = (exit_px - entry_px) * qty
                    closed  = True
                # Normal SL: bar dropped to stop intrabar
                elif mr["low"] <= sl_px:
                    exit_px = sl_px
                    gross   = (sl_px - entry_px) * qty
                    closed  = True
                # GAP-AWARE partial TP: bar opened AT or ABOVE pTP -> cap fill at pTP
                    ...
                    running_ext     = mr["open"]   # bar opened higher; anchor trail here
                ...
                # GAP-AWARE SL: bar opened AT or ABOVE stop -> fill at open (worse)
                if mr["open"] >= sl_px:
                    exit_px = mr["open"]
                ...
                # Normal SL
                    ...
                    closed  = True
                # GAP-AWARE partial TP: bar opened AT or BELOW pTP -> cap fill at pTP
                elif mr["open"] <= partial_tp_px:
                    partial_locked  = (entry_px - partial_tp_px) * half
                    fee_acc        += partial_tp_px * half * fee_side
                    slip_acc       += partial_tp_px * half * slip_pct
                    running_ext     = mr["open"]   # bar opened lower; anchor trail here
        ...
        # ── PARTIAL position ──────────────────────────────────────────────────
            ...
            if direction == "LONG":
                ...
                # GAP-AWARE: check bar open FIRST before updating running_ext
                if mr["open"] <= old_trail:              # gap DOWN through trail
                    exit_px = mr["open"]                  # worse fill (below trail)
                    gross   = partial_locked + (exit_px - entry_px) * half
                    closed  = True
                elif mr["open"] >= hard_tp_px:           # gap UP through hard TP
                    exit_px = hard_tp_px                  # conservative: cap at TP
                    gross   = partial_locked + (hard_tp_px - entry_px) * half
                    closed  = True
                else:
                    # Normal bar: update running_ext / trail, then check intrabar
                    running_ext = max(running_ext, mr["high"])
                    trail_sl    = max(entry_px,
                                       running_ext - TRAIL_DIST_MULT * trade_atr)
                    # Conservative: trail wins when both fire on the same bar
                    if mr["low"] <= old_trail:
                        exit_px = old_trail
                ...
                # GAP-AWARE: check bar open FIRST before updating running_ext
                if mr["open"] >= old_trail:              # gap UP through trail (SHORT)
                    exit_px = mr["open"]                  # worse fill (above trail)
                    gross   = partial_locked + (entry_px - exit_px) * half
                    closed  = True
                elif mr["open"] <= hard_tp_px:           # gap DOWN through hard TP (SHORT)
                    exit_px = hard_tp_px                  # conservative: cap at TP
...
def _exec_1h(df_1h, start_idx, entry_px, direction,
              ...
              trade_atr, qty, fee_side, slip_pct):
    """
    Identical logic to v7 _exec_1h — used for Forex (no 1m data).
    Gap-aware fix is NOT applied here (1h bars have different gap characteristics
    and Forex is not the focus of this v8 test).
...
def main():
    ...
    div("="); div(" ")
    print("  LIQUIDATION ZONE REVERSAL  v8  --  GAP-AWARE FILL MODE")
    print(f"  Generated : {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    div(" "); div("=")
    print(f"""
  v8 UPGRADE  (all v7 / v6 strict-mode fixes inherited)
    Signal detection   : 1h candles  (unchanged)
    Trade execution    : 1m candles  (unchanged)
    GAP-AWARE fills    : NEW -- when 1m bar opens beyond stop/target:
                         - Stops  : fill at bar OPEN (worse price)
                         - Targets: fill at TARGET   (conservative cap)
  ...
  STRATEGY
    ...
    SL / Partial TP    : {SL_MULT}x / {PARTIAL_TP_MULT}x ATR
    Trail / Hard Cap   : {TRAIL_DIST_MULT}x / {HARD_TP_MULT}x ATR
  ...
  STRICT MODE FIXES ACTIVE
    ...
    [FIX 8] Gap-aware fill : bar-open fill when gap-through (NEW in v8)
""")

    div("=")
    print("  CRYPTO  (1m execution + gap-aware fills)")
    div("=")
    ...
    div("=")
    print("  FOREX  (1h fallback -- gap-aware NOT applied)")
    div("=")
    ...
    # ─── v7 vs v8 comparison ──────────────────────────────────────────────────
    div("="); div(" ")
    print("  v7 vs v8  --  WHAT CHANGED")
    div(" "); div("=")
    print("""
  v7  filled ALL exits (SL, trail, partial TP, hard TP) at the EXACT
      stop/target level, even when a 1m bar gapped past that level.

  v8  adds gap-aware detection per 1m bar OPEN:
      - Bar opens AT or BEYOND a stop level  -> fill at bar OPEN (worse).
      - Bar opens AT or BEYOND a target      -> fill at TARGET (conservative).
      The conservative tie-break (trail > hard_tp) is unchanged.

  Expected impact (per forensic audit):
      ~1.6% of trades affected, ~0.3-0.4% lower net PnL vs v7.
      The underlying edge (Net PF > 2, CAGR 25-30%) remains intact.
""")

    csv = OUTPUT_DIR / "backtest_v8_full.csv"
